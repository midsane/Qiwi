...
export const Modal = ({ color=true, cong=false, children, handleClose, minH=0 }) => {
    return (createPortal(
        <div className="relative">
            ...
            <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1}}
                className={`flex rounded z-20 ${color && "bg-myellow"} p-4 w-fit h-fit flex-col fixed left-1/2 top-1/2 translate-x-[-50%] translate-y-[-50%] gap-4 justify-start items-center min-w-72 ${minH !== 0 && "min-h-96"}`} >
